...
def get_jmp_chart_data(
    ...
    result = (
        ...
        .filter(ViewDataScore.question == question)
        .filter(ViewDataScore.administration.in_(administration))
        .filter(ViewDataScore.data.in_(data))
        .group_by(ViewDataScore.administration)
        .group_by(ViewDataScore.option)
        .all()
    )
    ...
    for k, v in groupby(result, key=lambda x: x["administration"]):
        ...
        for kc, vc in groupby(child, key=lambda x: x["option"]):
            child_group.append(
                {"option": kc, "count": sum([x["count"] for x in list(vc)])}
            )
...
def get_pie_chart_data(
    ...
    answer = (
        ...
        .filter(Answer.data.in_(data))
        .filter(Answer.question == question)
        .group_by(Answer.options)
        .all()
    )
    options = (
    ...
    for a in answer:
        color = (
            default_color_config[a.options[0].lower()]
            if a.options[0].lower() in default_color_config
            else None
        )
        option = list(
...
def get_overviews_visualization(
    session: Session, form: int, question: int, option: str
):
    answer = (
        session.query(Answer.options, func.count(Answer.id).label("count"))
        .filter(Answer.question == question)
    ...
    for a in answer:
        ...
        color = (
            default_color_config[opt.lower()]
