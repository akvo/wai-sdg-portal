...
def validate_administration(session, answer, adm):
    ...
    name = adm["name"]
    if len(aw) < 2:
        return {"error_message": ValidationText.administration_validation.value}
    if aw[0] != adm["name"]:
        return {
            "error_message": f"{ValidationText.administration_not_valid.value} {name}"
        }
    ...
    for col in excel_head:
        ...
        if not error:
            ...
            for i, answer in enumerate(answers):
                ix = i + 2
                error = validate_row_data(session, f"{col}{ix}", answer, question, adm)
                if error:
